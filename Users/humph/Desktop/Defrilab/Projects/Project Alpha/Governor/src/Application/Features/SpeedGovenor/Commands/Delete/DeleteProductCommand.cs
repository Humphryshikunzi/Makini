...
namespace _.Application.Features.Products.Commands.Delete
{
    ...
    internal class DeleteProductCommandHandler : IRequestHandler<DeleteProductCommand, Result<int>>
    {
        ...
        {
            var product = await _unitOfWork.Repository<SpeedGovernor>().GetByIdAsync(command.Id);
            if (product != null)
            {
                await _unitOfWork.Repository<SpeedGovernor>().DeleteAsync(product);
