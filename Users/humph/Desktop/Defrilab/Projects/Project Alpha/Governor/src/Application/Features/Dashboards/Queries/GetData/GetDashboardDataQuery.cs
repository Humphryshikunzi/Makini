...
namespace _.Application.Features.Dashboards.Queries.GetData
{
    ...
    internal class GetDashboardDataQueryHandler : IRequestHandler<GetDashboardDataQuery, Result<DashboardDataResponse>>
    {
        ...
        public async Task<Result<DashboardDataResponse>> Handle(GetDashboardDataQuery query, CancellationToken cancellationToken)
        {
            ...
            {
                ProductCount = await _unitOfWork.Repository<SpeedGovernor>().Entities.CountAsync(cancellationToken),
            ...
            for (int i = 1; i <= 12; i++)
            {
                ...
                productsFigure[i - 1] = await _unitOfWork.Repository<SpeedGovernor>().Entities.Where(x => x.CreatedOn >= filterStartDate && x.CreatedOn <= filterEndDate).CountAsync(cancellationToken);
